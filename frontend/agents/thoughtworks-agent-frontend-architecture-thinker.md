...
4. **设计 Pages 与路由** — 页面作为组合层：
   ...
   - 列出路由路径和页面间导航关系
   - 说明每个页面消费哪些 API 端点
   - **绘制页面布局线框图**（ASCII 线框图）：标注 Header/侧边栏/主内容区/Footer 等区域，每个区域标注对应的 Feature/Widget 组件名称
   - **描述响应式策略**：断点、移动端适配方案（如侧边栏折叠、表格转卡片）
   - **描述页面交互流程**：用户操作 → 界面响应（弹窗/跳转/展开面板/动效），覆盖主要操作路径
...
| "页面布局是编码时再决定的" | 没有布局线框图的页面设计，Worker 不知道组件如何排列、区域如何划分，只能凭空猜测 |
