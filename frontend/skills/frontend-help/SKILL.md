...
- `agent-frontend-worker` — 用 Bash 运行 `frontend-status.mjs` 了解整体进度，完成编码后运行 `frontend-workflow-status.mjs --set-task <task_id> coded` 标记 task 完成，然后运行 `--sync-layer-status` 同步层级状态
