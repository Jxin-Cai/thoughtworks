...
1. 你的 skills 配置已自动注入 `frontend-spec` 技能。按照该技能的路由规则加载对应的规范文件。
...
4. **扫描 OHS 层已有代码** — 如需确认后端 API 端点，通过 Glob/Grep/Read 扫描已有 OHS 代码获取准确的接口定义
5. 用 Glob/Grep 工具探索项目结构
6. **输出实现方案** — 逐条列出要创建/修改的文件路径、关键实现点和创建顺序

### Phase B: 编码
7. 按实现方案的顺序，用 Write/Edit 工具创建或修改代码文件
