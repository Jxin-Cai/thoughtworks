...
Agent(
  ...
  prompt: "
    ...
    3. 验证通过 → 执行：
       bash {FRONTEND_HELP}/scripts/frontend-workflow-status.sh {IDEA_DIR} --finish-task {task_id} coded
       并用 Edit 将 task 文件 frontmatter 的 status 更新为 done
    4. 验证失败 → 执行：
       bash {FRONTEND_HELP}/scripts/frontend-workflow-status.sh {IDEA_DIR} --finish-task {task_id} failed
       并报告缺失的产物列表
  "
)
```
