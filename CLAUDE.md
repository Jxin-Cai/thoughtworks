...
- **根级 `thoughtworks-all`**：`skills` 字段声明 `["./skills/", "./backend/skills/", "./frontend/skills/"]`，`commands` 声明 `["./commands/", "./backend/commands/", "./frontend/commands/"]`，将所有技能和命令拉入 all 命名空间。`agents` 仍逐文件引用 backend/frontend agents（agents 按文件名注册为 subagent_type，不受插件命名空间影响）。
