...
Claude Code 通过 `.claude-plugin/plugin.json` 发现并加载本插件。根级 `plugin.json` 通过 `agents`、`skills`、`commands` 数组字段显式声明所有子目录路径，确保三层子插件的组件都能被正确注册。后端 agent 放在 `backend/agents/`，前端 agent 放在 `frontend/agents/`，各子插件独立安装时也能通过自身 `plugin.json` 的 `agents` 字段发现 agent。`hooks/session-start` 脚本在每次会话启动时注入技能触发索引到会话上下文。
