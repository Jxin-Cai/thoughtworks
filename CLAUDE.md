...
基于契约驱动设计的 Claude Code 插件，采用 backend/ + frontend/ + 根级三层子插件架构。后端通过多智能体协同生成 DDD 四层架构的 Java 代码，前端基于 OHS 导出契约独立闭环生成前端代码。设计与实现严格分离：Thinker agent 只产设计文档，Worker agent 只按设计写代码。跨层一致性通过导出契约与依赖契约的签名匹配来保证。支持领域拆分：大需求在澄清阶段通过 DDD 战略分析拆分为多个聚合上下文（迭代单元），每个上下文独立走完整的 Phase 循环，多上下文按 DAG 拓扑序串行执行。
...
后端 Decision-Maker 调用 `/thoughtworks-backend-clarify` 执行需求澄清、`/thoughtworks-branch` 管理功能分支、`/thoughtworks-backend-thought` 执行设计、`/thoughtworks-backend-works` 执行编码。按 Phase 串行编排：每个 Phase 内 Thinker→用户确认→Worker 串行执行，Phase 1（domain）→ Phase 2（infr + application 并行）→ Phase 3（ohs）。下游层的 Thinker 通过扫描已实现的上游代码获取依赖接口列表。

前端 Decision-Maker 调用 `/thoughtworks-frontend-clarify` 执行需求澄清、`/thoughtworks-branch` 管理功能分支、`/thoughtworks-frontend-thought` 执行设计、`/thoughtworks-frontend-works` 执行编码。前端依赖后端 OHS 层导出契约。

全栈编排器（`/thoughtworks-all`）直接调度澄清、分支管理、设计、编码子技能，不通过 Decision-Maker 中转。编排思路与各 Decision-Maker 一致，但独立闭环。
...
**契约驱动的跨层一致性。** 每个设计文档包含「导出契约」和「依赖契约」区块。下游层必须从上游层的导出契约逐条抄入依赖契约。后端 `backend-output-validate.sh` 执行签名匹配校验（规则 C1–C5）。前端 `frontend-output-validate.sh` 执行前端契约校验（C6：Frontend 依赖契约 ⊆ OHS 导出契约）。
...
**领域拆分在澄清技能中执行。** 后端澄清技能（`thoughtworks-skills-backend-clarify`）在需求澄清后执行 DDD 战略分析，将需求按聚合上下文拆分为多个迭代单元。一个聚合上下文 = 一个迭代单元 = 一个 idea 目录 + 一个 feature 分支。拆分后每个上下文独立走完整的 Phase 循环（评估 → 设计 → 编码），多上下文按 DAG 拓扑序串行执行。全栈场景下，每个后端上下文 OHS 完成后立即做该上下文的前端。下游 skill（thought/works/branch）不感知多上下文，只接收单个 idea-name。
...
├── requirement.md                # 后端需求存档（含元数据：所属领域拆分、上游依赖）
...
```
.thoughtworks/
├── .supplementary-tasks.md              # 全局工程支撑任务（全栈编排器使用）
├── product-management/                  # 上下文 1
│   ├── requirement.md                   # 含元数据：所属领域拆分、上游依赖
│   ├── assessment.md
│   ├── workflow-state.json
│   ├── .approved
│   ├── backend-designs/
│   │   └── ...
│   ├── frontend-requirement.md
│   ├── frontend-assessment.md
│   ├── frontend-workflow-state.json
│   ├── .frontend-approved
│   └── frontend-designs/
│       └── frontend.md
