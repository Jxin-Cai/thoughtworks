...
Claude Code 通过 `.claude-plugin/plugin.json` 发现并加载本插件，注册根级 `skills/` 和 `commands/`。所有 agent 定义文件统一放在根级 `agents/` 目录下（扁平结构，无子目录），Claude Code 按约定自动发现并注册。`hooks/session-start` 脚本在每次会话启动时将 `skills/using-thoughtworks/SKILL.md` 的完整内容注入会话上下文（包裹在 `<EXTREMELY_IMPORTANT>` 标签中），并附加从 `thoughtworks.yaml` 读取的当前 mode 信息。

## 架构：三层子插件

```
根级（thoughtworks-all）
├── agents/                        — 所有 agent 定义（扁平结构，Claude Code 按约定自动发现）
