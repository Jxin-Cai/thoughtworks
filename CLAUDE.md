...
Claude Code 通过 `.claude-plugin/plugin.json` 发现并加载本插件，注册根级 `skills/` 和 `commands/`。后端和前端各自有独立的 `.claude-plugin/plugin.json`，分别注册各自的 skills、agents、commands。`hooks/session-start` 脚本在每次会话启动时将 `skills/using-thoughtworks/SKILL.md` 的完整内容注入会话上下文（包裹在 `<EXTREMELY_IMPORTANT>` 标签中），并附加从 `thoughtworks.yaml` 读取的当前 mode 信息。
...
│   ├── agents/thinkers/                         — 后端设计 agent
│   ├── agents/workers/                          — 后端编码 agent
│   └── commands/                                — 后端命令
│
└── frontend/（thoughtworks-frontend）
    ...
    ├── agents/thinkers/                              — 前端设计 agent
    ├── agents/workers/                               — 前端编码 agent
    └── commands/                                     — 前端命令
```
...
**使用自定义 agent 类型，而非 general-purpose。** 每层有专属的 agent 定义文件（如 `thoughtworks-agent-ddd-domain-thinker`），frontmatter 配置了 `skills: [thoughtworks-skills-java-spec]`。SKILL.md 必须用这些 agent 名称作为 `subagent_type`，这样 agent body（system prompt）和 skills 会自动加载。动态 prompt 只需包含 MISSION、TEMPLATE、CONTEXT、OUTPUT，禁止重复内联 INSTRUCTION 或 CODING-SPEC。前端同理：`thoughtworks-agent-frontend-thinker` 和 `thoughtworks-agent-frontend-worker` 的 frontmatter 配置了 `skills: [thoughtworks-skills-frontend-spec]`。
...
- `thinker-ref` / `worker-ref` → agent `.md` 文件（文件名即 `subagent_type`）
