...
**UI/UX 实现增强。** 前端 Worker agent 在编码阶段集成 `ui-ux-pro-max` 技能。编码前主 agent 检测该技能是否可用，若可用则运行 `search.py --design-system` 生成设计系统（色彩、字体、样式 token），运行 `--stack {tech}` 获取技术栈最佳实践，将输出注入 Worker prompt 的 CONTEXT 区。Worker 编码时按设计系统 token 编写样式，完成前执行 UI/UX 检查清单。设计阶段（Thinker）的 UI/UX 指引保持不变，作为需求层面的设计参考。
