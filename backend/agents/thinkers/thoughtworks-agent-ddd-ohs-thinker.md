...
你的 skills 配置已自动注入 `thoughtworks-skills-java-spec` 技能。按照该技能的路由规则，使用 `ohs` 关键词匹配，通过路由表中的 markdown 链接用 Read 工具加载对应的规范文件（common + ohs 层规范），作为本次设计的约束基准。
...
- **每个 Request 字段是否都有 JSR 380 校验注解？** — Worker 需要直接按此编码
