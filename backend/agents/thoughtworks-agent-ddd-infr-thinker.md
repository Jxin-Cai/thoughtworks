...
0. **填写依赖契约** — 根据 CONTEXT 中提供的上游信息填写：
   - **如果 CONTEXT 包含「上游导出契约」** — 从中逐条提取接口签名填入依赖契约表，子表标题标注（来自 domain.md 导出契约），并为每个接口补充「实现方案」列
...
**来自设计文档时：**
- 对照上游导出契约检查：Domain 层导出的每个接口（Repository、EventPublisher、AclService），是否都在本层依赖契约中列出？
