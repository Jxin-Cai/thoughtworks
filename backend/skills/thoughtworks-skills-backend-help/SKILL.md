...
- `scripts/backend-workflow-status.sh` — 工作流状态管理脚本，支持查看状态、非阻塞检查上游层完成(--check-upstream)、全部完成(--check-all)、设置层状态(--set)、获取层状态(--get-status)、状态转换合法性校验
...
- `thoughtworks-agent-ddd-thinker` — 用 Read 工具加载 workflow.yaml 了解层间依赖，加载 workflow-state.yaml 确认上游完成状态
