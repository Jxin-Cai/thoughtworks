# Application 层 Task 设计文档模板

> 每个 task 文件对应一组相关的业务用例（按用例组分）。
...
order: {N}
status: pending
depends_on: [{上游 task_id 列表，如 domain-001, domain-002}]
description: "{用例组名称}"
---
...
（一句话概括：本 task 要做什么，涉及哪些业务用例）

<!-- REQUIRED -->
## 依赖契约

> 以下接口和对象定义来自 Domain 层，Application 层作为消费方使用。
> 来源为已有代码（标注源文件路径），按聚合分组。
...
##### Repository 接口（来自已有代码）

| 接口名 | 方法签名 | 返回类型 | 本层用途 |
|--------|---------|---------|---------|
| {AggregateRoot}Repository | `{方法签名}` | {返回类型} | {如：加载聚合根 / 持久化状态变更} |
...
<!-- 返回类型必须是领域层模型（聚合根、实体、值对象或其组合），禁止在 Application 层定义 DTO。 -->
...
|------|-----------|---------------|-----------|---------|
| 1 | Output_Application_{IdeaName}_{nnn}_01 | `{package}.{ClassName}` | {从上方设计中提取的2-5个关键实现要点} | {对应的章节名} |
```
