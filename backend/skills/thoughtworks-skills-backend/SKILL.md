...
| 有 idea，无 designs | `requirement.md` 存在，`backend-designs/` 为空 | → Step 3 评估 → Phase 循环编排 |
...
2. **判断 `$ARGUMENTS` 是恢复旧 idea 还是全新需求**：
   ...
   - 如果 `$ARGUMENTS` 是一段需求描述文本（而非 idea 目录名）→ 全新需求，走 Step 1 → Step 2 澄清
   - 如果不确定，使用 AskUserQuestion 向用户确认：是要恢复已有 idea，还是开始新的需求
3. 恢复旧 idea 时，检查 `requirement.md` 是否存在：**不存在则从 Step 2 澄清开始**，存在则根据上表决定续传位置
4. 全新需求始终从 Step 1 → Step 2 开始
...
5. `requirement.md` 不存在 → 从 Step 2 澄清开始（补完未完成的澄清）
