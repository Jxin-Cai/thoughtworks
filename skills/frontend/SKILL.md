...
1. **只做前端** — 即使需求描述涉及后端，也只生成前端代码，不调用任何后端技能。如需前后端联动，提示用户调用 `/all` 入口
...
3. 确定 idea-dir：
   ...
   - 如果不存在，idea-dir = `none`
4. **运行编排状态检查**：`node {SCRIPTS}/orchestration-status.mjs <idea-dir> frontend`
...
```
LOOP:
  1. result = node {SCRIPTS}/orchestration-status.mjs <idea-dir> frontend
