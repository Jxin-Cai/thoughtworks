...
4. **填写导出契约** — 汇总应用服务 API、Command 定义、返回类型定义，供 Worker 实现和契约校验引用
...
- **Command 字段定义清晰吗？** — Worker 需要按此编码，字段类型和含义是否无歧义？
- **方法签名够具体吗？** — Worker 实现时，参数和返回值是否完全确定？

### 步骤 3: 上游契约一致性验证

- 仅在验证失败时按需读取说明列标注的源文件中的相关方法签名段落，不要全文重读。确认依赖契约中记录的方法签名确实存在
